...
class BuildActionTests: XCTestCase {
  ...
  func testImportObject() {
    ...
    XCTAssertEqual(action.buildActionID, "2014-10-31T01:40:13.046Z infrastructure Start container 0 1")
  ...
  func testImportObject2() {
    ...
    XCTAssertEqual(action.buildActionID, "2014-10-31T01:41:04.988Z test NODE_ENV=production npm run-script compile 0 20")
    XCTAssertEqual(Int(BuildAction.MR_countOfEntities()), 1, "1 record exists")
    BuildAction.MR_importFromObject(obj)
    BuildAction.MR_importFromObject(obj)
    XCTAssertEqual(Int(BuildAction.MR_countOfEntities()), 1, "1 record exists")
